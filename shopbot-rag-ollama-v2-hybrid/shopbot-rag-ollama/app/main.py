...
# ═══════════════════════════════════════════════════════════
# COMPARE — dense vs BM25 vs hybrid affiancati
# ═══════════════════════════════════════════════════════════

def cmd_compare(query: str, top_k: int = 5):
    print(f'\n🔬 Confronto retrieval per: "{query}"\n')

    try:
        dense_results  = cerca_prodotti(query, top_k=top_k)
        bm25_results   = cerca_prodotti_bm25(query, top_k=top_k)
        hybrid_results = cerca_prodotti_hybrid(query, top_k=top_k)
    except Exception as e:
        print(f"❌ Errore: {e}")
        sys.exit(1)

    def stampa_colonna(titolo, risultati, campo_score, label_score):
        print(f"  {titolo}")
        print(f"  {'─' * 38}")
        if not risultati:
            print("    (nessun risultato)")
        for i, p in enumerate(risultati, 1):
            score = p.get(campo_score, "—")
            print(f"  {i}. {p['nome']:<32}")
            print(f"     {label_score}: {score}   €{p['prezzo']:.2f}")
        print()

    stampa_colonna("DENSE (ChromaDB / coseno)", dense_results, "distanza", "dist")
    stampa_colonna("BM25 (lessicale)",          bm25_results,  "score",    "score")
    stampa_colonna("HYBRID (RRF)",              hybrid_results,"score_rrf","rrf")

    # Evidenzia le differenze: prodotti trovati solo da un sistema
    nomi_dense  = {p["nome"] for p in dense_results}
    nomi_bm25   = {p["nome"] for p in bm25_results}
    solo_dense  = nomi_dense - nomi_bm25
    solo_bm25   = nomi_bm25 - nomi_dense

    if solo_dense or solo_bm25:
        print("  📊 Differenze tra i due sistemi:")
        if solo_dense:
            print(f"     Solo nel dense: {', '.join(solo_dense)}")
        if solo_bm25:
            print(f"     Solo in BM25:   {', '.join(solo_bm25)}")
        print()
...
def cmd_stats():
    ...
    print(f"    USE_HYBRID   : {config.USE_HYBRID}  (BM25 + Dense via RRF)" if config.USE_HYBRID
          else f"    USE_HYBRID   : {config.USE_HYBRID}  (solo dense retrieval)")
    print()
...
def main():
    ...
    # compare
    p_compare = sub.add_parser("compare", help="Confronta dense vs BM25 vs hybrid")
    p_compare.add_argument("query", help='Testo da cercare, es. "Salomon X Ultra"')
    p_compare.add_argument("--top-k", type=int, default=5, dest="top_k",
                           help="Numero max di risultati per sistema (default: 5)")
    ...
    if args.command == "setup":
        ...
        cmd_compare(args.query, top_k=args.top_k)
