...
> Se scrivi `django-admin startproject negozio_django` senza il `.`
> Django crea due cartelle con lo stesso nome, una dentro l'altra:
>
> ```
> negozio_django/        ← cartella esterna
>   ├── negozio_django/  ← cartella di configurazione (stesso nome!)
>   │   ├── settings.py
>   │   └── urls.py
>   └── manage.py
> ```
>
> Puoi anche scrivere `django-admin startproject negozio_django .` con il `.`
> e il problema della cartella doppia viene risolto, ma hai ancora due cose
> che si chiamano `negozio_django` — la cartella in cui sei e quella di configurazione.
> Usando `config` come nome ottieni una struttura più pulita e senza ambiguità:
>
> ```
> negozio_django/
>   ├── config/          ← subito chiaro che è la configurazione
>   │   ├── settings.py
>   │   └── urls.py
>   └── manage.py
> ```
>
> In sostanza il `.` risolve il problema della cartella doppia, ma il nome `config` risolve anche il problema della confusione visiva.
...
Una list comprehension è un modo compatto per scrivere un ciclo `for` su una sola riga.
...
```python
# Ciclo for classico — 3 righe
for p in risultato:
    print(p.nome, p.prezzo)

# List comprehension — 1 riga, fa la stessa cosa
[print(p.nome, p.prezzo) for p in risultato]
```

Le parentesi quadre `[ ]` dicono a Python che stai costruendo una lista.
`print()` restituisce sempre `None`, quindi vedrai `[None, None, None]` stampato dopo i risultati — è normale, non è un errore.

Se invece vuoi costruire una vera lista di valori (non solo stamparli):

```python
# Lista dei soli nomi
nomi = [p.nome for p in risultato]
# → ['T-shirt', 'Jeans', 'Cuffie']

# Lista di tuple nome + prezzo
dati = [(p.nome, p.prezzo) for p in risultato]
# → [('T-shirt', 25.00), ('Jeans', 59.00), ('Cuffie', 79.00)]
```
...
```python
#            ↓ cosa fai        ↓ variabile  ↓ da dove prendi i dati
[print(p.nome, p.prezzo)  for p  in risultato]
```
...
**Spiegazione ORM:**
- `filter(prezzo__range=(10, 100))` → `WHERE prezzo BETWEEN 10 AND 100`
- `.order_by('prezzo')` → `ORDER BY prezzo ASC`. Con `'-prezzo'` sarebbe `DESC`
...
```sql
SELECT negozio_prodotto.*
FROM negozio_prodotto
INNER JOIN negozio_categoria ON negozio_prodotto.categoria_id = negozio_categoria.id
WHERE negozio_categoria.nome = 'Elettronica'
  AND negozio_prodotto.in_stock = 1
  AND negozio_prodotto.prezzo <= 500;
```

**Spiegazione ORM:**
- `categoria__nome='Elettronica'` → naviga la ForeignKey con `__`, in SQL diventa una `INNER JOIN`
- `in_stock=True` → `AND in_stock = 1`
- `.exclude(prezzo__gt=500)` → esclude prezzi > 500€, equivalente a `filter(prezzo__lte=500)`
...
> La list comprehension non si applica qui perché `.count()` restituisce già
> un singolo intero, non una lista da iterare.
...
**Spiegazione ORM:**
- `filter(prodotto__id=3)` → `WHERE prodotto_id = 3`
- `.count()` → `SELECT COUNT(*)`, più efficiente di `len(queryset)` perché non carica i dati in memoria
...
**Ciclo for classico** (`.aggregate()` restituisce un dizionario, si itera con `.items()`):
...
```python
#          ↓ cosa fai           ↓ variabili    ↓ da dove prendi i dati
...
**Spiegazione ORM:**
- `.aggregate()` → restituisce un dizionario, non un queryset
- `Avg`, `Min`, `Max` vanno importate da `django.db.models`
...
> Nota: `.values()` restituisce dizionari invece di oggetti modello,
> quindi si accede ai campi con `p['nome']` invece di `p.nome`
...
**Spiegazione ORM:**
- `.order_by('-prezzo')` → `ORDER BY prezzo DESC` (il `-` inverte l'ordine)
- `.values('nome', 'prezzo')` → `SELECT nome, prezzo` (solo i campi richiesti)
- `[:5]` → `LIMIT 5`
